...
const CMS_QUERY_SETTINGS = (page) =>
	`a=1&ty=article&per_page=50&page=${page || 1}`
...
export const discoverArticlesByTag = (dispatch) => (tag, page) => {
	dispatch({
		type: ActionTypes.DISCOVER_ARTICLES_BY_TAG.REQUEST,
		reset: page === 1,
	})
	new Promise((resolve, reject) => {
		axios
			...
			.then((response) => {
				dispatch({
					type: ActionTypes.DISCOVER_ARTICLES_BY_TAG.SUCCESS,
					payload: {
...
export const searchArticles = (dispatch) => (query, page) => {
	...
	new Promise((resolve, reject) => {
		axios
			...
			.then((response) => {
				dispatch({
					type: ActionTypes.SEARCH_ARTICLES.SUCCESS,
					payload: {
...
export const getBookmarks = (dispatch) => async () => {
	...
	try {
		...
		const res = await axios.get(`${ROOT_URL}/articles/bookmarks/${userId}`)
		const populatedBookmarks = await Promise.all(
			res.data.map(
				({ slug }) =>
					new Promise((resolve) => {
						axios
							.get(`${CMS_URL}/article/${slug}.json`)
							.then((articleRes) =>
								resolve(types.articleConverter(articleRes.data.article))
							)
					})
			)
		)
		dispatch({
			type: ActionTypes.GET_BOOKMARKS.SUCCESS,
			payload: {
 ...
 */
export const readArticle = (dispatch) => (article) =>
	new Promise(async (resolve) => {
		dispatch({ type: ActionTypes.READ_ARTICLE.REQUEST, payload: article })
		const userId = await SecureStore.getItemAsync('userId')
		axios
			.post(`${ROOT_URL}/articles/reads`, {
			...
			.then(() => {
				axios
					.get(
						`${ROOT_URL}/articles/${article.slug}${
							userId ? `?for=${userId}` : ''
						}`
					)
					.then((metaArticleRes) => {
						dispatch({
							type: ActionTypes.READ_ARTICLE.SUCCESS,
							payload: { ...article, ...metaArticleRes.data },
						})
						resolve()
					})
			})
...
export const exitArticle = (dispatch) => () =>
	dispatch({ type: ActionTypes.EXIT_ARTICLE })
...
export const bookmarkArticle = (dispatch) => async (articleSlug) => {
	dispatch({
		type: ActionTypes.BOOKMARK_ARTICLE.REQUEST,
		payload: articleSlug,
	})
	try {
		const userId = await SecureStore.getItemAsync('userId')
		...
		const res = await axios.post(`${ROOT_URL}/articles/bookmarks/${userId}`, {
			articleSlug,
		})
		switch (res.status) {
			case 201:
		...
		console.log(err)
		dispatch({
			type: ActionTypes.BOOKMARK_ARTICLE.FAILURE,
			payload: articleSlug,
		})
	}
}
